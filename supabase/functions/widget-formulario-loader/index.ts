...
function getFormScript(configApi: string, submitApi: string, mode: string, slug: string, anonKey: string): string {
  ...
  parts.push("function inputType(t){switch(t){case'email':return'email';case'telefone':case'telefone_br':return'tel';case'numero':case'decimal':case'moeda':return'number';case'url':return'url';case'data':return'date';case'data_hora':return'datetime-local';case'hora':return'time';default:return'text'}}")
  ...
  parts.push("if(tipo==='cep')return function(el){el.setAttribute('maxlength','9');el.addEventListener('input',function(){var v=this.value.replace(/\\\\D/g,'');if(v.length>8)v=v.slice(0,8);if(v.length>5)this.value=v.slice(0,5)+'-'+v.slice(5);else this.value=v})};")
  ...
  parts.push("function renderField(c,group,labelCss,inputCss,fontFamily,borderColor){")
  parts.push("if(isLayoutOnly(c.tipo))return;")
  parts.push("var label=document.createElement('label');label.className='form-label';label.textContent=c.label+(c.obrigatorio?' *':'');label.style.cssText=labelCss;group.appendChild(label);")
  parts.push("var inp;")
  ...
  parts.push("else if((c.tipo==='select'||c.tipo==='dropdown'||c.tipo==='selecao')&&c.opcoes){inp=document.createElement('select');inp.style.cssText=inputCss;var defOpt=document.createElement('option');defOpt.value='';defOpt.textContent=c.placeholder||'Selecione...';inp.appendChild(defOpt);var opts=Array.isArray(c.opcoes)?c.opcoes:(c.opcoes.opcoes||[]);for(var j=0;j<opts.length;j++){var o=document.createElement('option');var val=typeof opts[j]==='string'?opts[j]:(opts[j].valor||opts[j].label||opts[j]);var lbl=typeof opts[j]==='string'?opts[j]:(opts[j].label||opts[j].valor||opts[j]);o.value=val;o.textContent=lbl;inp.appendChild(o)}}")
  parts.push("else if(c.tipo==='selecao_multipla'&&c.opcoes){inp=document.createElement('div');inp.setAttribute('data-name',c.nome);inp.setAttribute('data-multi','1');var mOpts=Array.isArray(c.opcoes)?c.opcoes:(c.opcoes.opcoes||[]);for(var m=0;m<mOpts.length;m++){var mLabel=document.createElement('label');mLabel.style.cssText='display:flex;align-items:center;gap:8px;font-size:14px;color:#374151;cursor:pointer;padding:8px 12px;border:1px solid '+(fS.input_border_color||'#D1D5DB')+';border-radius:6px;margin-bottom:6px;transition:all 0.15s;background:#fff;font-family:'+fontFamily;var mInp=document.createElement('input');mInp.type='checkbox';mInp.style.cssText='width:16px;height:16px;accent-color:#3B82F6;flex-shrink:0';var mVal=typeof mOpts[m]==='string'?mOpts[m]:(mOpts[m].valor||mOpts[m].label||mOpts[m]);mInp.value=mVal;mInp.addEventListener('change',function(){var p=this.parentElement;if(this.checked){p.style.borderColor='#3B82F6';p.style.backgroundColor='#EFF6FF'}else{p.style.borderColor=fS.input_border_color||'#D1D5DB';p.style.backgroundColor='#fff'}});mLabel.appendChild(mInp);mLabel.appendChild(document.createTextNode(typeof mOpts[m]==='string'?mOpts[m]:(mOpts[m].label||mOpts[m].valor||mOpts[m])));inp.appendChild(mLabel)}}")
  parts.push("else if(c.tipo==='checkbox'){inp=document.createElement('input');inp.type='checkbox';inp.style.cssText='width:18px;height:18px'}")
  parts.push("else if(c.tipo==='radio'&&c.opcoes){inp=document.createElement('div');inp.setAttribute('data-name',c.nome);var rOpts=Array.isArray(c.opcoes)?c.opcoes:(c.opcoes.opcoes||[]);for(var r=0;r<rOpts.length;r++){var rLabel=document.createElement('label');rLabel.style.cssText='display:flex;align-items:center;gap:6px;font-size:14px;color:#374151;cursor:pointer;margin-bottom:4px';var rInp=document.createElement('input');rInp.type='radio';rInp.name=c.nome;var rVal=typeof rOpts[r]==='string'?rOpts[r]:(rOpts[r].valor||rOpts[r].label||rOpts[r]);rInp.value=rVal;rLabel.appendChild(rInp);rLabel.appendChild(document.createTextNode(typeof rOpts[r]==='string'?rOpts[r]:(rOpts[r].label||rOpts[r].valor||rOpts[r])));inp.appendChild(rLabel)}}")
  parts.push("else{inp=document.createElement('input');inp.type=inputType(c.tipo);inp.style.cssText=inputCss}")
  parts.push("if(inp.tagName!=='DIV'){inp.className='form-input-field';inp.name=c.nome;if(c.placeholder)inp.placeholder=c.placeholder;if(c.obrigatorio)inp.required=true;inp.addEventListener('focus',function(){this.style.borderColor='#3B82F6'});inp.addEventListener('blur',function(){this.style.borderColor=borderColor});var mk=maskFor(c.tipo);if(mk)mk(inp)}")
  parts.push("group.appendChild(inp);if(c.texto_ajuda){var help=document.createElement('span');help.textContent=c.texto_ajuda;help.style.cssText='font-size:11px;color:#9ca3af;margin-top:2px;display:block';group.appendChild(help)}}")
  ...
  parts.push("if(c.tipo==='titulo'){var tc=parseLayoutConfig(c.valor_padrao,'titulo');var tEl=document.createElement('h3');tEl.textContent=c.placeholder||c.label;tEl.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box;font-size:'+tc.tamanho+'px;font-weight:600;margin:0;text-align:'+tc.alinhamento+';color:'+tc.cor+';font-family:'+fontFamily;formEl.appendChild(tEl);continue}")
  parts.push("if(c.tipo==='paragrafo'){var pc=parseLayoutConfig(c.valor_padrao,'paragrafo');var pEl=document.createElement('p');pEl.textContent=c.placeholder||c.label;pEl.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box;font-size:'+pc.tamanho+'px;margin:0;text-align:'+pc.alinhamento+';color:'+pc.cor+';font-family:'+fontFamily;formEl.appendChild(pEl);continue}")
  parts.push("if(c.tipo==='divisor'){var dc=parseLayoutConfig(c.valor_padrao,'divisor');var hr=document.createElement('hr');hr.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box;border:none;border-top:'+dc.espessura+'px '+dc.estilo+' '+dc.cor;formEl.appendChild(hr);continue}")
  ...
  parts.push("if(c.tipo==='bloco_html'){var htmlDiv=document.createElement('div');htmlDiv.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box;font-size:14px;font-family:'+fontFamily;htmlDiv.innerHTML=c.valor_padrao||'';formEl.appendChild(htmlDiv);continue}")
  parts.push("if(c.tipo==='imagem_link'){var imgWrap=document.createElement('div');imgWrap.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box';try{var imgP=JSON.parse(c.valor_padrao||'{}');var isMob=window.innerWidth<768;var isTab=window.innerWidth>=768&&window.innerWidth<1024;var imgUrl=(isMob&&imgP.url_mobile)||(isTab&&imgP.url_tablet)||imgP.url_desktop||imgP.url_tablet||imgP.url_mobile||'';var linkUrl=imgP.redirect_url||'';if(imgUrl){var imgEl=document.createElement('img');imgEl.src=imgUrl;imgEl.alt=c.label;imgEl.style.cssText='width:100%;border-radius:8px;display:block';if(linkUrl){var a=document.createElement('a');a.href=linkUrl;a.target='_blank';a.rel='noopener noreferrer';a.appendChild(imgEl);imgWrap.appendChild(a)}else imgWrap.appendChild(imgEl)}}catch(e){}formEl.appendChild(imgWrap);continue}")
  parts.push("if(c.tipo==='oculto')continue;if(isLayoutField(c.tipo))continue;")

  // Column blocks
  parts.push("if(c.tipo==='bloco_colunas'){var colWrap=document.createElement('div');colWrap.style.cssText='width:100%;padding:'+fieldPadding+';box-sizing:border-box';try{var cp=JSON.parse(c.valor_padrao||'{}');var ncols=parseInt(cp.colunas)||2;var larguras=(cp.larguras||'50%,50%').split(',');var gap=cp.gap||'16';var colContainer=document.createElement('div');colContainer.setAttribute('data-bloco-id',c.id);colContainer.style.cssText='display:flex;flex-wrap:wrap;gap:'+gap+'px';")
  ...
  parts.push("var children=campos.filter(function(ch){return ch.pai_campo_id===c.id&&ch.coluna_indice===ci}).sort(function(a,b){return a.ordem-b.ordem});for(var cj=0;cj<children.length;cj++){var child=children[cj];var cDiv=document.createElement('div');var cw=child.largura==='1/2'||child.largura==='half'?'50%':child.largura==='1/3'||child.largura==='third'?'33.33%':child.largura==='2/3'?'66.66%':'100%';cDiv.style.cssText='width:'+cw+';padding:'+fieldPadding+';box-sizing:border-box';renderField(child,cDiv,labelCss,inputCss,fontFamily,bc);col.appendChild(cDiv)}")
  parts.push("colContainer.appendChild(col)}colWrap.appendChild(colContainer)}catch(e){}formEl.appendChild(colWrap);continue}")

  // Regular fields
  parts.push("var larguraMap={full:'100%','1/2':'50%','1/3':'33.33%','2/3':'66.66%',half:'50%',third:'33.33%'};var fw=larguraMap[c.largura]||'100%';var group=document.createElement('div');group.style.cssText='width:'+fw+';padding:'+fieldPadding+';box-sizing:border-box';renderField(c,group,labelCss,inputCss,fontFamily,bc);formEl.appendChild(group)}")
  ...
  // Submit handler
  parts.push("formEl.addEventListener('submit',function(e){e.preventDefault();if(enviarBtn){enviarBtn.disabled=true;enviarBtn.textContent='Enviando...';enviarBtn.style.opacity='0.7'}var formData={};var inputs=formEl.querySelectorAll('input,textarea,select');for(var k=0;k<inputs.length;k++){var inp=inputs[k];if(inp.name&&inp.name[0]!=='_'){if(inp.type==='checkbox'&&!inp.closest('[data-multi]'))formData[inp.name]=inp.checked;else if(inp.type==='radio'){if(inp.checked)formData[inp.name]=inp.value}else formData[inp.name]=inp.value}}var radioDivs=formEl.querySelectorAll('div[data-name]:not([data-multi])');for(var r=0;r<radioDivs.length;r++){var checked=radioDivs[r].querySelector('input[type=radio]:checked');if(checked)formData[radioDivs[r].getAttribute('data-name')]=checked.value}var multiDivs=formEl.querySelectorAll('div[data-multi]');for(var m=0;m<multiDivs.length;m++){var name=multiDivs[m].getAttribute('data-name');var checkedBoxes=multiDivs[m].querySelectorAll('input[type=checkbox]:checked');var vals=[];for(var mc=0;mc<checkedBoxes.length;mc++)vals.push(checkedBoxes[mc].value);formData[name]=vals.join('|')}")
