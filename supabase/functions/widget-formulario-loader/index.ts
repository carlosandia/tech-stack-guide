...
function getFormScript(configApi: string, submitApi: string, mode: string, slug: string, anonKey: string): string {
  ...
  parts.push("if(c.tipo!=='checkbox'&&c.tipo!=='checkbox_termos'){var label=document.createElement('label');label.className='form-label';label.textContent=c.label+(c.obrigatorio?' *':'');label.style.cssText=labelCss;group.appendChild(label)}")
  parts.push("var inp;")
  ...
  parts.push("else if(c.tipo==='telefone'){var telWrap=document.createElement('div');telWrap.style.cssText='display:flex;align-items:center;gap:0';var ddiSel=document.createElement('select');ddiSel.name=c.nome+'_ddi';ddiSel.style.cssText=inputCss+';width:auto;min-width:90px;border-right:none;border-top-right-radius:0;border-bottom-right-radius:0;background:#F3F4F6;padding:8px 6px;appearance:auto;flex-shrink:0';for(var di=0;di<PAISES_DDI.length;di++){var dOpt=document.createElement('option');dOpt.value=PAISES_DDI[di].d;dOpt.textContent=PAISES_DDI[di].f+' '+PAISES_DDI[di].d;ddiSel.appendChild(dOpt)}var telInp=document.createElement('input');telInp.type='tel';telInp.className='form-input-field';telInp.name=c.nome;if(c.placeholder)telInp.placeholder=c.placeholder;else telInp.placeholder='Digite aqui...';if(c.obrigatorio)telInp.required=true;telInp.style.cssText=inputCss+';border-top-left-radius:0;border-bottom-left-radius:0';telInp.addEventListener('focus',function(){this.style.borderColor='#3B82F6'});telInp.addEventListener('blur',function(){this.style.borderColor=borderColor});telWrap.appendChild(ddiSel);telWrap.appendChild(telInp);group.appendChild(telWrap);if(c.texto_ajuda){var help=document.createElement('span');help.textContent=c.texto_ajuda;help.style.cssText='font-size:11px;color:#9ca3af;margin-top:2px;display:block';group.appendChild(help)}return}")

  // TELEFONE BR (com prefixo fixo 🇧🇷 +55)
  parts.push("else if(c.tipo==='telefone_br'){var telBrWrap=document.createElement('div');telBrWrap.style.cssText='display:flex;align-items:center;gap:0';var prefix=document.createElement('span');prefix.textContent='\\uD83C\\uDDE7\\uD83C\\uDDF7 +55';prefix.style.cssText=inputCss+';width:auto;border-right:none;border-top-right-radius:0;border-bottom-right-radius:0;display:flex;align-items:center;gap:4px;padding:8px 10px;background:#F3F4F6;color:#6B7280;font-size:14px;white-space:nowrap;flex-shrink:0;height:auto';var telBrInp=document.createElement('input');telBrInp.type='tel';telBrInp.className='form-input-field';telBrInp.name=c.nome;telBrInp.placeholder=c.placeholder||'(00) 00000-0000';if(c.obrigatorio)telBrInp.required=true;telBrInp.style.cssText=inputCss+';border-top-left-radius:0;border-bottom-left-radius:0';telBrInp.addEventListener('focus',function(){this.style.borderColor='#3B82F6'});telBrInp.addEventListener('blur',function(){this.style.borderColor=borderColor});var mk=maskFor('telefone_br');if(mk)mk(telBrInp);telBrWrap.appendChild(prefix);telBrWrap.appendChild(telBrInp);group.appendChild(telBrWrap);if(c.texto_ajuda){var help=document.createElement('span');help.textContent=c.texto_ajuda;help.style.cssText='font-size:11px;color:#9ca3af;margin-top:2px;display:block';group.appendChild(help)}return}")
  ...
  // CHECKBOX
  parts.push("else if(c.tipo==='checkbox'){var cbWrap=document.createElement('div');cbWrap.style.cssText='display:flex;align-items:center;gap:8px';var cbInp=document.createElement('input');cbInp.type='checkbox';cbInp.name=c.nome;cbInp.style.cssText='width:16px;height:16px;accent-color:#3B82F6';var cbLabel=document.createElement('span');cbLabel.style.cssText=labelCss+';margin-bottom:0';cbLabel.textContent=c.label+(c.obrigatorio?' *':'');cbWrap.appendChild(cbInp);cbWrap.appendChild(cbLabel);group.appendChild(cbWrap);if(c.texto_ajuda){var help=document.createElement('span');help.textContent=c.texto_ajuda;help.style.cssText='font-size:11px;color:#9ca3af;margin-top:2px;display:block';group.appendChild(help)}return}")

  // CHECKBOX TERMOS
  parts.push("else if(c.tipo==='checkbox_termos'){var ctWrap=document.createElement('div');ctWrap.style.cssText='display:flex;align-items:flex-start;gap:8px';var ctInp=document.createElement('input');ctInp.type='checkbox';ctInp.name=c.nome;if(c.obrigatorio)ctInp.required=true;ctInp.style.cssText='width:16px;height:16px;margin-top:2px;accent-color:#3B82F6;flex-shrink:0';var ctLabel=document.createElement('span');ctLabel.style.cssText=labelCss+';margin-bottom:0;font-size:13px';ctLabel.textContent=c.label+(c.obrigatorio?' *':'');ctWrap.appendChild(ctInp);ctWrap.appendChild(ctLabel);group.appendChild(ctWrap);return}")
  ...
  parts.push("if(inp)group.appendChild(inp);if(c.texto_ajuda){var help=document.createElement('span');help.textContent=c.texto_ajuda;help.style.cssText='font-size:11px;color:#9ca3af;margin-top:2px;display:block';group.appendChild(help)}}")
