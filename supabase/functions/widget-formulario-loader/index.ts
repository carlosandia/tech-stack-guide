...
function getFormScript(configApi: string, submitApi: string, mode: string, slug: string, anonKey: string): string {
  ...
  // Masks — AIDEV-NOTE: Use \\D (single backslash in output JS) to match non-digits
  ...
  // ASSINATURA (placeholder area)
  parts.push("else if(c.tipo==='assinatura'){inp=document.createElement('div');inp.setAttribute('data-name',c.nome);var sigArea=document.createElement('div');sigArea.style.cssText=inputCss.replace(/height:[^;]+;/,'')+';height:80px;display:flex;align-items:center;justify-content:center;color:#9CA3AF;border-style:dashed;cursor:crosshair';sigArea.textContent='\\u00c1rea de assinatura';inp.appendChild(sigArea)}")
  ...
  // ARQUIVO / UPLOAD types
  parts.push("else if(c.tipo==='arquivo'||c.tipo==='imagem'||c.tipo==='documento'||c.tipo==='upload_video'||c.tipo==='upload_audio'){inp=document.createElement('div');inp.setAttribute('data-name',c.nome);var upArea=document.createElement('div');upArea.style.cssText=inputCss.replace(/height:[^;]+;/,'')+';padding:12px;text-align:center;color:#9CA3AF;border-style:dashed;cursor:pointer';upArea.textContent='Clique ou arraste para enviar';inp.appendChild(upArea)}")
