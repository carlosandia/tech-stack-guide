...
function getFormScript(configApi: string, submitApi: string, mode: string, slug: string, anonKey: string): string {
  ...
  parts.push("if(c.tipo!=='checkbox'&&c.tipo!=='checkbox_termos'){var label=document.createElement('label');label.className='form-label';label.style.cssText=labelCss+';display:flex;align-items:center;gap:4px';label.innerHTML=c.label+(c.obrigatorio?' <span style=\"color:#EF4444\">*</span>':'')+(c.texto_ajuda?'<span title=\"'+c.texto_ajuda.replace(/\"/g,'&quot;')+'\" style=\"cursor:help;display:inline-flex\">'+helpSvg+'</span>':'');group.appendChild(label)}")
  parts.push("var inp;")
  ...
  // CHECKBOX
  parts.push("else if(c.tipo==='checkbox'){var cbWrap=document.createElement('div');cbWrap.style.cssText='display:flex;align-items:center;gap:8px';var cbInp=document.createElement('input');cbInp.type='checkbox';cbInp.name=c.nome;cbInp.style.cssText='width:16px;height:16px;accent-color:#3B82F6';var cbLabel=document.createElement('span');cbLabel.style.cssText=labelCss+';margin-bottom:0';cbLabel.textContent=c.label+(c.obrigatorio?' *':'');cbLabel.innerHTML=c.label+(c.obrigatorio?' <span style=\"color:#EF4444\">*</span>':'')+(c.texto_ajuda?'<span title=\"'+c.texto_ajuda.replace(/\"/g,'&quot;')+'\" style=\"cursor:help;display:inline-flex;margin-left:4px\">'+helpSvg+'</span>':'');cbWrap.appendChild(cbInp);cbWrap.appendChild(cbLabel);group.appendChild(cbWrap);return}")

  // CHECKBOX TERMOS
  parts.push("else if(c.tipo==='checkbox_termos'){var ctWrap=document.createElement('div');ctWrap.style.cssText='display:flex;align-items:flex-start;gap:8px';var ctInp=document.createElement('input');ctInp.type='checkbox';ctInp.name=c.nome;if(c.obrigatorio)ctInp.required=true;ctInp.style.cssText='width:16px;height:16px;margin-top:2px;accent-color:#3B82F6;flex-shrink:0';var ctLabel=document.createElement('span');ctLabel.style.cssText=labelCss+';margin-bottom:0;font-size:13px';ctLabel.innerHTML=c.label+(c.obrigatorio?' <span style=\"color:#EF4444\">*</span>':'')+(c.texto_ajuda?'<span title=\"'+c.texto_ajuda.replace(/\"/g,'&quot;')+'\" style=\"cursor:help;display:inline-flex;margin-left:4px\">'+helpSvg+'</span>':'');ctWrap.appendChild(ctInp);ctWrap.appendChild(ctLabel);group.appendChild(ctWrap);return}")
