...
-- Corrigir trigger emitir_evento_automacao: classificar ganho/perda pelo tipo da etapa, não pelo motivo_resultado_id
-- AIDEV-NOTE: motivo_resultado_id pode estar preenchido em oportunidades ganhas (exigir_motivo_resultado = true)
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF v_entidade_tipo = 'oportunidade' THEN
      ...
      -- AIDEV-NOTE: Oportunidade ganha/perdida - CORRIGIDO para usar tipo da etapa
      -- Antes usava motivo_resultado_id IS NULL (incorreto pois ganhos tambem podem ter motivo)
      IF OLD.fechado_em IS NULL AND NEW.fechado_em IS NOT NULL THEN
        -- Consultar o tipo da etapa para determinar se é ganho ou perda
