...
BEGIN
  ...
  -- METAS DA EMPRESA
  SELECT
    COALESCE(json_agg(row_to_json(me) ORDER BY me.percentual DESC), '[]'::json),
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE me.percentual >= 100)::int,
    COALESCE(AVG(me.percentual), 0),
    COUNT(*) FILTER (WHERE me.percentual < 40 AND me.percentual > 0)::int
  INTO v_metas_empresa, v_total_metas, v_metas_atingidas, v_media_atingimento, v_em_risco
  FROM (
    SELECT
      m.nome,
      m.metrica,
  ...
  -- Incluir metas individuais no resumo
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE sub.percentual >= 100)::int,
    COALESCE(SUM(sub.percentual), 0),
    COUNT(*) FILTER (WHERE sub.percentual < 40 AND sub.percentual > 0)::int
  INTO v_total_ind, v_metas_atingidas, v_soma_ind, v_em_risco
  FROM (
    SELECT
      CASE WHEN m.valor_meta > 0
        THEN ROUND((COALESCE(mp.valor_atual, 0) / m.valor_meta) * 100, 1)
  ...
  -- Recalcular totais
  v_total_metas := v_total_metas + v_total_ind;
  v_metas_atingidas := v_metas_atingidas + (SELECT COUNT(*)::int FROM (
    ...
    WHERE m.organizacao_id = p_organizacao_id AND m.tipo = 'empresa' AND m.ativo = true AND m.deletado_em IS NULL
      AND m.data_inicio <= p_periodo_fim::date AND m.data_fim >= p_periodo_inicio::date
      AND CASE WHEN m.valor_meta > 0 THEN ROUND((COALESCE(mp.valor_atual, 0) / m.valor_meta) * 100, 1) ELSE 0 END >= 100
  ) x);
  v_em_risco := v_em_risco + (SELECT COUNT(*)::int FROM (
    SELECT 1 FROM metas m
    LEFT JOIN LATERAL (SELECT valor_atual FROM metas_progresso WHERE meta_id = m.id ORDER BY calculado_em DESC LIMIT 1) mp ON true
    WHERE m.organizacao_id = p_organizacao_id AND m.tipo = 'empresa' AND m.ativo = true AND m.deletado_em IS NULL
      AND m.data_inicio <= p_periodo_fim::date AND m.data_fim >= p_periodo_inicio::date
      AND CASE WHEN m.valor_meta > 0 THEN ROUND((COALESCE(mp.valor_atual, 0) / m.valor_meta) * 100, 1) ELSE 0 END < 40
      AND CASE WHEN m.valor_meta > 0 THEN ROUND((COALESCE(mp.valor_atual, 0) / m.valor_meta) * 100, 1) ELSE 0 END > 0
  ) x);

  IF v_total_metas > 0 THEN
    v_media_atingimento := ROUND((v_media_atingimento * (v_total_metas - v_total_ind) + v_soma_ind) / v_total_metas, 1);
  END IF;
