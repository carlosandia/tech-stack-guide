...
CREATE OR REPLACE FUNCTION fn_dashboard_metricas_gerais(
...
BEGIN
  ...
  -- AIDEV-NOTE: Total histórico SEM filtro de período (todas oportunidades já criadas)
  ...
  -- AIDEV-NOTE: Total no período (para calcular variação de crescimento)
  ...
  FROM (
    SELECT
      ...
      COUNT(*) AS quantidade,
      CASE WHEN v_perdas > 0 THEN ROUND(COUNT(*)::numeric / v_perdas::numeric * 100, 1) ELSE 0 END AS percentual
    ...
    WHERE o.organizacao_id = p_organizacao_id
      ...
      AND o.fechado_em >= p_periodo_inicio AND o.fechado_em <= p_periodo_fim
      AND o.deletado_em IS NULL
      AND (p_funil_id IS NULL OR o.funil_id = p_funil_id)
    GROUP BY mr.nome, mr.cor ORDER BY COUNT(*) DESC LIMIT 10
  ) mp;
  ...
  FROM (
    SELECT
      ...
      COUNT(*) AS quantidade,
      CASE WHEN v_ganhos > 0 THEN ROUND(COUNT(*)::numeric / v_ganhos::numeric * 100, 1) ELSE 0 END AS percentual
    FROM oportunidades o
    LEFT JOIN motivos_resultado mr ON mr.id = o.motivo_resultado_id
    WHERE o.organizacao_id = p_organizacao_id
  ...
  FROM (
    SELECT p.nome AS nome, SUM(op.quantidade)::bigint AS quantidade, SUM(op.subtotal)::numeric AS receita
    ...
    WHERE o.organizacao_id = p_organizacao_id
      ...
      AND o.deletado_em IS NULL
      AND (p_funil_id IS NULL OR o.funil_id = p_funil_id)
    GROUP BY p.nome ORDER BY SUM(op.quantidade) DESC LIMIT 5
  ) pr;
  ...
  RETURN json_build_object(
    ...
    'total_oportunidades_periodo', v_total_periodo,
