...
import { Loader2, Zap, Eye, EyeOff, CheckCircle2, AlertTriangle, Lock } from 'lucide-react'
 ...
 * 
 * Dados do pré-cadastro são reaproveitados e exibidos como somente leitura.
 * O usuário precisa apenas definir a senha.
 */
...
export function OnboardingPage() {
  ...
  const [hasPreCadastro, setHasPreCadastro] = useState(false)
  ...
  useEffect(() => {
    async function fetchSession() {
      ...
      try {
        ...
        // AIDEV-NOTE: Se há pré-cadastro, pré-preencher todos os campos
        if (data?.pre_cadastro) {
          const pc = data.pre_cadastro as PreCadastro
          setHasPreCadastro(true)

          // Split nome_contato em nome e sobrenome
          const partes = (pc.nome_contato || '').trim().split(/\s+/)
          const nome = partes[0] || ''
          const sobrenome = partes.slice(1).join(' ') || ''

          setValue('nome_empresa', pc.nome_empresa || '')
          setValue('segmento', pc.segmento || '')
          setValue('admin_nome', nome)
          setValue('admin_sobrenome', sobrenome)
          setValue('admin_email', pc.email || data.customer_email || '')
          setValue('admin_telefone', pc.telefone || '')
  ...
  // Estilo de campo bloqueado (somente leitura)
  const lockedInputClass = "w-full px-3 py-2 text-sm bg-muted border border-input rounded-md text-muted-foreground cursor-not-allowed"
  // Estilo de campo editável
  const editableInputClass = "w-full px-3 py-2 text-sm bg-background border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent"
        ...
        <div className="max-w-lg mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground mb-2">
              {hasPreCadastro ? 'Defina sua senha' : 'Complete seu cadastro'}
            </h1>
            <p className="text-muted-foreground">
              {hasPreCadastro
                ? 'Seus dados foram recuperados do cadastro anterior. Defina sua senha para continuar.'
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="nome_empresa" className="block text-sm font-medium text-foreground mb-1.5">
                    Nome da Empresa {!hasPreCadastro && <span className="text-destructive">*</span>}
                    {hasPreCadastro && <Lock className="inline w-3 h-3 ml-1 text-muted-foreground" />}
                  </label>
                  <input
                    id="nome_empresa"
                    type="text"
                    {...register('nome_empresa')}
                    readOnly={hasPreCadastro}
                    className={hasPreCadastro ? lockedInputClass : editableInputClass}
                ...
                <div>
                  ...
                  {hasPreCadastro ? (
                    <input
                      ...
                      type="text"
                      readOnly
                      value={SEGMENTOS_ONBOARDING.find(s => s.value === form.getValues('segmento'))?.label || form.getValues('segmento') || 'Não informado'}
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="admin_nome" className="block text-sm font-medium text-foreground mb-1.5">
                      Nome {!hasPreCadastro && <span className="text-destructive">*</span>}
                      {hasPreCadastro && <Lock className="inline w-3 h-3 ml-1 text-muted-foreground" />}
                    </label>
                    <input
                      id="admin_nome"
                  ...
                  <div>
                    <label htmlFor="admin_sobrenome" className="block text-sm font-medium text-foreground mb-1.5">
                      Sobrenome {!hasPreCadastro && <span className="text-destructive">*</span>}
                      {hasPreCadastro && <Lock className="inline w-3 h-3 ml-1 text-muted-foreground" />}
                    </label>
                    <input
                      id="admin_sobrenome"
                ...
                <div>
                  <label htmlFor="admin_email" className="block text-sm font-medium text-foreground mb-1.5">
                    Email <Lock className="inline w-3 h-3 ml-1 text-muted-foreground" />
                  </label>
                  <input
                    id="admin_email"
                    type="email"
                ...
                <div>
                  <label htmlFor="admin_telefone" className="block text-sm font-medium text-foreground mb-1.5">
                    Telefone {!hasPreCadastro && <span className="text-destructive">*</span>}
                    {hasPreCadastro && <Lock className="inline w-3 h-3 ml-1 text-muted-foreground" />}
                  </label>
                  <input
                    id="admin_telefone"
                    ...
                    readOnly={hasPreCadastro}
                    onChange={hasPreCadastro ? undefined : (e) => {
