...
export function useEnviarMedia() {
  ...
  return useMutation({
    ...
    // AIDEV-NOTE: Optimistic Update — mídia aparece instantaneamente no chat
    onMutate: async (variables) => {
      await queryClient.cancelQueries({ queryKey: ['mensagens', variables.conversaId] })
      const snapshot = queryClient.getQueryData(['mensagens', variables.conversaId])
      ...
      const optimisticMsg = {
        ...
        to_number: null,
        participant: null,
        tipo: variables.dados.tipo as any,
        body: variables.dados.caption || null,
        media_url: variables.dados.localPreview || variables.dados.media_url,
